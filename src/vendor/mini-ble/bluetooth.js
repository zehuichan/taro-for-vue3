...
export default class Bluetooth {
    ...
    if (!this.isInitializedAdapter) {
      ...
      Taro.onBluetoothAdapterStateChange((res) => {
        ...
        if (res.available && !this.isAvailableAdapter) {
          ...
          Taro.getEnv() === 'ALIPAY' && (this.isInitializedAdapter = false)
          this.callBackConnectStatus(
      ...
      return Taro.closeBluetoothAdapter()
        .then(() => Taro.openBluetoothAdapter())
        .then((res) => {
        ...
        Taro.onBluetoothDeviceFound((res) => {
          ...
          for (let device of devices) {
            ...
            if (typeof device.advertisData !== 'string')
              device.advertisData = _.ab2str(device.advertisData)
            // 上报设备
            this.callBackDiscoverDevice(
              device,
  ...
  stopScan() {
    ...
    return Taro.stopBluetoothDevicesDiscovery()
      .then(() => {
        this.bm.log('stopBluetoothDevicesDiscovery success')
  ...
  connect(device, timeout) {
    ...
    return this.openAndListenBluetoothAdapter()
      .then(() => {
        return Taro.createBLEConnection({
          deviceId,
        ...
        Taro.getBLEDeviceServices({ deviceId })
          .then((res) => {
            ...
            device.services = res.services.map((server) => {
              ...
              let sPromise = Taro.getBLEDeviceCharacteristics({
                deviceId,
                serviceId: sUUID
              })
          ...
          .catch((e) => {
            ...
            Taro.closeBLEConnection({ deviceId: this.bm.deviceInfo.deviceId })
              .then()
              .catch()
            this.bm.connectStatus = ConnectStatus.disconnected
      ...
      .catch((e) => {
        ...
        // 未知错误，直接报连接失败
        if (e.code === 100000)
          e = ErrorCallbackEvent.Error_ConnectStatus_CB_ConnectFail
        this.bm.connectStatus = ConnectStatus.disconnected
        this.callBackConnectStatus(e)
        if (e.code === 12 || e.code === 10001) {
          return Promise.reject(ErrorApiCatch.Error_Connect_PowerOff)
        } else if (
          ~e.message.indexOf('超时') ||
          (e.errMsg && ~e.errMsg.indexOf('time out'))
        ) {
  ...
  read(params) {
    if (this.bm.connectStatus === ConnectStatus.connected) {
      ...
      return Taro.readBLECharacteristicValue({
